...
function EditTeacher({com,teacher,setActivemodal}) {
    ...
    const uploadCroppedImage = async () => {
        try {
            ...
            setPreviousImages([croppedData.filePath]);

            message('Изображение успешно загружены');
        } catch (error) {
    ...
    const createPost = async () => {
        try{
            if(namepost.length>0 && descpost.length>0){
                ...
                const {data} = await NewsService.editTeacherAvia({id: teacher.id,name: namepost, dev: descpost, desc: view, image: imgpage, capter: com})
                if(data){
                    setView([])
                    setNamepost('')
                    setDescpost('')
    ...
    useEffect(()=>{
        if(teacher){
            setView(teacher.desc)
            setNamepost(teacher.name)
            setDescpost(teacher.dev)
            setImgpage(teacher.photo)
        }
