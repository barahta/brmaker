...
function EditGroupCom({news, setActivemodal}) {
    const coms = [
        {
            name: 'OMEDIA!',
            active: false
    ...
    const createGroupCom = async () => {
        try{
            ...
            const logo = imglogo
            const img = imgpage
            const number = numposition

            const {data} = await NewsService.editСomGroup({id,name,desc,contacts,site,logo,img,number})
            if(data){
                setActivemodal(false)
                message(`О! ${namepost} успешно Изменено`)
                setNamepost('')
                setDescpost('')
                setUrlsite('')
                setImgpage('')
    ...
    useEffect(()=>{
        setViewcom(coms)
        setNamepost(news.name)
        setDescpost(news.desc)
        setImgpage(news.image)
        setImglogo(news.logo)
        setUrlsite(news.site)
        setListContact(news.contacts)
        setNumposition(+news.number)
    }, [news])
    return (
            ...
            <div className={style.main}>
                <div className={style.left}>
                    ...
                    <div
                        ...
                        style={(imglogo && imglogo.length>0)?{backgroundImage: `url('${imglogo}')`}:{backgroundImage: `url('/files/news/nophoto.svg')`}}>
                ...
                <div className={style.right}>
                    <div className={style.up}>
                        ...
                        <div className={style.listcom}>
                            {(listContact)&&listContact.map((contact, indexcontact)=>(
