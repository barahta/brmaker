...
function EditPrice({pack, setPack, com, setActivemodal}){
    ...
    const createPrice = async () => {
        ...
        if(name.length > 2){
            try{
                const {data} = await NewsService.editPriceAvia({id: pack.id,capter: com, name, description: time, priceour: [ot1, priceour], priceyour: [ot2, priceyour], time: timeline, theory: theory, practice, programs: programlist, discounts: alldiscount})
                if(data){
                    setName('')
                    setTime('')
                    setPriceour('')
    ...
    useEffect(() => {
        if(pack && pack!== ''){
            setName(pack.name)
            setTime(pack.description)
            setOt1(pack.priceour[0])
            setPriceour(pack.priceour[1])
            setOt2(pack.priceyour[0])
            setPriceyour(pack.priceyour[1])
            setTimeline(pack.time)
            setTheory(pack.theory)
            setPractice(pack.practice)
            setProgramlist(pack.programs)
            setAllDiscount(pack.discounts)
        }
    ...
    return(
        <div className={style.main}>
            ...
            <div className={style.line_btn} onClick={createPrice}>Сохранить</div>
        </div>
    )
}
