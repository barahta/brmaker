    ...
    const createPost = async () => {
        try{
            if(textpost.length>0 && namepost.length>0 && descpost.length>0){
                ...
                const {data} = await NewsService.createPost({text: [textpost], name: namepost, desc: descpost, image: imgpage, public: viewcom, imagefull: imgfull, newsDateTime, publishDateTime})
                if(data){
                    setView([])
                    setNamepost('')
                    setDescpost('')
